...
class SpecReporter extends _reporter.default {
  ...
  printReport(runner) {
    ...
    const output = [...this.getHeaderDisplay(runner), ...results, ...this.getCountDisplay(duration), ...this.getFailureDisplay(), ...this.getTestLink(runner)]; // Prefix all values with the browser information
  ...
  getTestLink({
    ...
    if (config.hostname.includes('saucelabs')) {
      const dc = config.headless ? '.us-east1.headless' : ['eu', 'eu-central-1'].includes(config.region) ? '.eu-central-1' : '';
  ...
  getResultDisplay() {
    ...
    for (const suite of suites) {
      ...
      for (const test of eventsToReport) {
        ...
        const testIndent = `${this.defaultTestIndent}${suiteIndent}`; // Output for a single test
  ...
  getFailureDisplay() {
    ...
    for (const suite of suites) {
      ...
      for (const test of eventsToReport) {
        ...
        const testTitle = test.title;
        const errors = test.errors || [test.error]; // If we get here then there is a failed test

        output.push('', `${++failureLength}) ${suiteTitle} ${testTitle}`);

        for (let error of errors) {
          output.push(this.chalk.red(error.message), ...error.stack.split(/\n/g).map(value => this.chalk.gray(value)));
        }
      }
    }
